...
tile(Monitor *m)
{
	...
	if (n > m->nmaster)
		mw = m->nmaster ? (m->w.width + gappx*ie) * m->mfact : 0;
	else
		mw = m->w.width - 2*gappx*oe + gappx*ie;
	i = 0;
	my = ty = gappx*oe;
	wl_list_for_each(c, &clients, link) {
		if (!VISIBLEON(c, m) || c->isfloating || c->isfullscreen)
		...
		if (i < m->nmaster) {
			r = MIN(n, m->nmaster) - i;
			h = (m->w.height - my - gappx*oe - gappx*ie * (r - 1)) / r;
			resize(c, (struct wlr_box){.x = m->w.x + gappx*oe, .y = m->w.y + my,
				.width = mw - gappx*ie, .height = h}, 0, draw_borders);
			my += c->geom.height + gappx*ie;
		} else {
			r = n - i;
			h = (m->w.height - ty - gappx*oe - gappx*ie * (r - 1)) / r;
			resize(c, (struct wlr_box){.x = m->w.x + mw + gappx*oe, .y = m->w.y + ty,
				.width = m->w.width - mw - 2*gappx*oe, .height = h}, 0, draw_borders);
			ty += c->geom.height + gappx*ie;
		}
		i++;
	}
}
